...
from redroid_automation import check_gemini_offer_redroid
                    ...
                    if status == "needs_totp":
                        ...
                        if totp_secret:
                            try:
                                ...
                                if not offer_link:
                                    logger.info("Web automation found no offer link. Trying Android Emulator flow via redroid_automation...")
                                    try:
                                        redroid_link = await asyncio.to_thread(
                                            check_gemini_offer_redroid,
                                            email_str,
                                            pw_str,
                                            device,
                                            totp_secret,
                                        ...
                                        if redroid_link:
                                            logger.info("Android Emulator flow captured offer link: %s", redroid_link)
                                            offer_link = redroid_link
                                    except Exception as redroid_err:
                                        logger.warning("Android Emulator flow attempt failed: %s", redroid_err)
